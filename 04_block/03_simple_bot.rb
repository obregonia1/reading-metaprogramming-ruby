...
class SimpleBot
  def ask(name)
    self.class.instance_variable_get(:@hash)[name].call if self.class.instance_variable_get(:@hash)&.key?(name)
  end
  ...
  def self.setting(key, value)
    self.class.define_singleton_method key do
      value
    end
  end

  def self.respond(key, &block)
    if (hash = instance_variable_get(:@hash))
      hash[key] = block
    else
      instance_variable_set(:@hash, { key => block })
    end
  end
end
